...
public class DoxygentXMLParserTest {
	...
	public void testParseDoxygenXmlFile() {
		...
		try {
			...
			DoxygentXMLParser.parseDoxygenXmlFile(fileName, compound);
			// logger.info("compound member size=" + compound.getMembers().size());
			...
			assertEquals("CWE15_External_Control_of_System_or_Configuration_Setting__w32_22_badSource",
					member.getName());
			...
			DoxygenLocation location = member.getLocation();
			assertEquals(
			...
			DoxygenReference referencedby = member.getReferencedby()
					.get("testcases_8h_1a5c68fe5c6a56d9f243f06fe949cfe485");
			assertEquals("CWE15_External_Control_of_System_or_Configuration_Setting__w32_22_bad",
